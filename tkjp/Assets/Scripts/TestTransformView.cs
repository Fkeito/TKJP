...
namespace Asset.Scripts
{
    ...
    public class TestTransformView : MonoBehaviour, IPunObservable
    {
...
//                transform.rotation = Quaternion.RotateTowards(transform.rotation, this.m_NetworkRotation, this.m_Angle * (1.0f / PhotonNetwork.SerializationRate));
                transform.position = Vector3.MoveTowards(transform.position, Vector3.Lerp(transform.position,m_NetworkPosition,0.1f),this.m_Distance * (1.0f / PhotonNetwork.SerializationRate));
            }
        }
